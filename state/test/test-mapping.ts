...
function testMapping(mapping, ...cases) {
  for (let i = 0; i < cases.length; i++) {
    let [from, to, {bias = 1, trackDel = false} = {}] = cases[i]
...
describe("Mapping", () => {
  ...
  it("can map through a single deletion", () => {
    testMapping(mk([2, 6, 0]), [0, 0], [2, 2, {bias: -1}], [3, 2], [6, 2], [6, 2, {bias: -1}], [7, 3])
  })

  it("can map through a single replace", () => {
    testMapping(mk([2, 6, 4]), [0, 0], [2, 2], [4, 6], [4, 2, {bias: -1}], [6, 6, {bias: -1}], [8, 8])
  })
